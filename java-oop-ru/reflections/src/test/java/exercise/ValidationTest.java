...
class ValidationTest {
    ...
    void testAdvancedValidate() {
        ...
        expected1.put("country", new ArrayList<>(List.of("length less than 4")));
        expected1.put("street", new ArrayList<>(List.of("can not be null")));
        assertThat(result1).isEqualTo(expected1);

        Address address2 = new Address("Russia", "no", "street", null, "2");
        Map<String, List<String>> result2 = Validator.advancedValidate(address2);
        Map<String, List<String>> expected2 = new HashMap<>();
        expected2.put("city", new ArrayList<>(List.of("length less than 3")));
        expected2.put("houseNumber", new ArrayList<>(List.of("can not be null")));
        assertThat(result2).isEqualTo(expected2);
    }
    // END
}
